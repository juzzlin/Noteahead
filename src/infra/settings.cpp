...
int gainStagingTargetDb(int defaultGainStagingTargetDb)
{
    QSettings settings;
    settings.beginGroup(settingsGroupAudio);
    const auto targetDb = settings.value(gainStagingTargetDbKey, defaultGainStagingTargetDb).toInt();
    settings.endGroup();
    return targetDb;
}
...
void setGainStagingTargetDb(int gainStagingTargetDb)
{
    QSettings settings;
    settings.beginGroup(settingsGroupAudio);
    settings.setValue(gainStagingTargetDbKey, gainStagingTargetDb);
    settings.endGroup();
}
...
int paletteAccentBlend(int defaultPaletteAccentBlend)
{
    QSettings settings;
    settings.beginGroup(settingsGroupTheme);
    const auto blend = settings.value(paletteAccentBlendKey, defaultPaletteAccentBlend).toInt();
    settings.endGroup();
    return blend;
}
