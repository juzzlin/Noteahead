...
void MidiInRtMidi::updateAvailableDevices()
{
...
void MidiInRtMidi::openDevice(MidiDeviceCR device)
{
    if (!m_ports.contains(device.portIndex())) {
        if (auto && midiIn = std::make_unique<RtMidiIn>(); device.portIndex() >= midiIn->getPortCount()) {
            ...
            m_ports[device.portIndex()] = std::move(midiIn);
        }
    }
}
...
{
    if (const auto index = device.portIndex(); m_ports.contains(index)) {
        m_callbacks[index] = std::move(callback);
        m_ports[index]->setCallback(&MidiInRtMidi::staticCallback, this);
        m_ports[index]->ignoreTypes(false, false, false); // Enable all types
    } else {
...
void MidiInRtMidi::staticCallback(double deltaTime, MessageP message, void * userData)
{
    ...
    if (userData && message) {
        ...
        for (auto & [index, midiIn] : self->m_ports) {
