...
#include "sndfile_backend.hpp"

namespace noteahead {

SndFileBackend::SndFileBackend() = default;
...
bool SndFileBackend::open(const std::string & filePath, Mode mode, Info & info)
{
    close();
...
int64_t SndFileBackend::readFloat(std::span<float> data)
{
...
int64_t SndFileBackend::readDouble(std::span<double> data)
{
...
int64_t SndFileBackend::readInt(std::span<int32_t> data)
{
...
int64_t SndFileBackend::writeFloat(std::span<const float> data)
{
...
int64_t SndFileBackend::writeInt(std::span<const int32_t> data)
{
...
bool SndFileBackend::seek(int64_t frames, int whence)
{
    return m_sndFile ? (sf_seek(m_sndFile, static_cast<sf_count_t>(frames), whence) != -1) : false;
}

bool SndFileBackend::isOpen() const
{
    return m_sndFile != nullptr;
}

AudioFileBackend::Info SndFileBackend::info() const
{
    return {
        m_sfInfo.frames,
        m_sfInfo.samplerate,
