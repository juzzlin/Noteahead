...
void TrackSettingsModelTest::test_setAndGet_sideChainSettings_shouldUpdateModel()
{
    TrackSettingsModel model;
    ...
    // 2. Verify toInstrument()
    auto instrument = model.toInstrument();
    const auto & sideChainOut = instrument->settings().midiEffects.midiSideChain;

    QCOMPARE(sideChainOut.enabled, true);
    QCOMPARE(sideChainOut.sourceTrackIndex, quint8 { 1 });
    QCOMPARE(sideChainOut.sourceColumnIndex, quint8 { 2 });
    QCOMPARE(sideChainOut.lookahead.count(), 10);
    QCOMPARE(sideChainOut.release.count(), 100);

    QCOMPARE(sideChainOut.targets.at(0).enabled, true);
    QCOMPARE(sideChainOut.targets.at(0).controller, quint8 { 80 });
    QCOMPARE(sideChainOut.targets.at(0).targetValue, quint8 { 127 });
    QCOMPARE(sideChainOut.targets.at(0).releaseValue, quint8 { 0 });

    // 3. Verify setInstrumentData()
    Instrument newInstrument("Test Port 2");
    auto settings = newInstrument.settings();
    auto & sideChainIn = settings.midiEffects.midiSideChain;
    sideChainIn.enabled = true;
    sideChainIn.sourceTrackIndex = 3;
    sideChainIn.sourceColumnIndex = 4;
    sideChainIn.lookahead = std::chrono::milliseconds(20);
    sideChainIn.release = std::chrono::milliseconds(200);
    sideChainIn.targets.resize(model.sideChainTargetCount()); 
    sideChainIn.targets.at(1) = {true, 90, 100, 10}; // Target 1
    newInstrument.setSettings(settings);

    model.setInstrumentData(newInstrument);

    QCOMPARE(model.sideChainEnabled(), true);
    QCOMPARE(model.sideChainSourceTrack(), quint8 { 3 });
    QCOMPARE(model.sideChainSourceColumn(), quint8 { 4 });
    QCOMPARE(model.sideChainLookahead(), 20);
    QCOMPARE(model.sideChainRelease(), 200);

    QCOMPARE(model.sideChainTargetEnabled(1), true);
    QCOMPARE(model.sideChainTargetController(1), quint8 { 90 });
    QCOMPARE(model.sideChainTargetTargetValue(1), quint8 { 100 });
    QCOMPARE(model.sideChainTargetReleaseValue(1), quint8 { 10 });
}
