...
#include "string_voice_v3_test.hpp"
...
void renderFrames(StringVoiceV3Device & dev, uint32_t frames, uint32_t sampleRate = 44100)
{
...
{
    StringVoiceV3Device dev { "Test StringVoice" };
...
void StringVoiceV3Test::test_midiNoteOn_shouldActivateAudio()
{
    StringVoiceV3Device dev { "Test StringVoice" };
...
void StringVoiceV3Test::test_midiNoteOff_shouldDecayToSilence()
{
    StringVoiceV3Device dev { "Test StringVoice" };
...
void StringVoiceV3Test::test_polyphony_shouldSupportMultipleSimultaneousNotes()
{
    ...
    StringVoiceV3Device devOne { "Test StringVoice One" };
...
void StringVoiceV3Test::test_polyphony_shouldNotClipWithManyNotes()
{
...
void StringVoiceV3Test::test_allNotesOff_shouldSilenceAllVoices()
{
    StringVoiceV3Device dev { "Test StringVoice" };
...
void StringVoiceV3Test::test_serialization_shouldRestoreParameters()
{
    QByteArray data;
    QBuffer buffer { &data };
...

    {
        ...
        StringVoiceV3Device dev { "Restored StringVoice" };
...
void StringVoiceV3Test::test_audio_output_not_zero()
{
    StringVoiceV3Device dev { "Test StringVoice" };
    dev.setSampleRate(44100);
    dev.processMidiNoteOn(60, 100);
...
void StringVoiceV3Test::test_realtimeCallbacks_shouldNotBeSilencedByEnsemble()
{
...
void StringVoiceV3Test::test_vibrato_shouldModulatePitchWithinASingleBuffer()
{
...
void StringVoiceV3Test::test_ensembleMode_shouldSupportChorusIPlusII()
{
    StringVoiceV3Device dev { "Test StringVoice" };
...
void StringVoiceV3Test::test_voiceRegisters_shouldFollowTheKeyboardSplit()
{
    ...
    auto renderPeak = [](StringVoiceV3Device & dev) {
    ...
    auto onlyRegister = [](StringVoiceV3Device & dev, float male8, float male4, float upperMale8, float female4) {
        dev.setStringsUpper(false);
        dev.setStringsLower(false);
    ...
    StringVoiceV3Device male4Lower { "Male4 lower" };
    ...
    StringVoiceV3Device male4Upper { "Male4 upper" };
    ...
    StringVoiceV3Device femaleUpper { "Female4 upper" };
    ...
    StringVoiceV3Device femaleLower { "Female4 lower" };
    ...
    StringVoiceV3Device upperMale { "UpperMale8" };
    ...
    StringVoiceV3Device silentDev { "Silent" };
...
void StringVoiceV3Test::test_stringsSwitches_shouldFollowTheKeyboardSplit()
{
...
//! Magnitude of @p buffer at exactly @p frequency over the given window. One Goertzel bin.
double binMagnitude(const std::vector<double> & buffer, uint32_t startFrame, uint32_t frames, double frequency, uint32_t sampleRate)
{
    const double omega { 2.0 * std::numbers::pi * frequency / sampleRate };
    double re { 0.0 };
    double im { 0.0 };
...
//! Where the fundamental's energy sits within @p window, in cents from @p f0.
//!
//! The centroid rather than the tallest bin: the Strings sound as several copies a few cents apart,
//! and which of them happens to be tallest flips back and forth as they beat. Their centre of mass
//! does not, and that is what a listener follows as the pitch.
double fundamentalCentroidCents(const std::vector<double> & buffer, uint32_t startFrame, uint32_t frames, double f0, uint32_t sampleRate)
{
    constexpr int bins { 41 };
    constexpr double spanCents { 100.0 };
    double weighted { 0.0 };
    double total { 0.0 };
    for (int bin = 0; bin < bins; bin++) {
        const double cents { -spanCents + 2.0 * spanCents * bin / (bins - 1) };
        const double magnitude { binMagnitude(buffer, startFrame, frames, f0 * std::exp2(cents / 1200.0), sampleRate) };
        weighted += cents * magnitude;
        total += magnitude;
    }
    return total > 0.0 ? weighted / total : 0.0;
}

//! A device set up to sound its Strings section alone, with everything that could modulate the
//! pitch from elsewhere switched off.
void setUpStringsOnly(StringVoiceV3Device & dev, uint32_t sampleRate)
{
...
void StringVoiceV3Test::test_strings_withEnsembleOff_shouldStillBeModulated()
{
    // The bug this device exists to fix. On the hardware the Strings shimmer whether the Ensemble
    // switch is engaged or not, because the section sounds as several detuned copies of itself. V2
    // had no such thing and shared the one Ensemble across both sections, so with the switch off its
    // strings stood at exactly their nominal pitch and sounded synthetic.
    //
    // Detuned copies beat against one another, so the level in the fundamental band rises and falls.
    // That is what is measured here: a section that has gone still would hold a flat level.
    constexpr uint32_t sampleRate { 44100 };
    StringVoiceV3Device dev { "Strings" };
    setUpStringsOnly(dev, sampleRate);
    dev.setEnsembleEnabled(false);
    dev.processMidiNoteOn(60, 100);

    constexpr uint32_t frames { 131072 };
    std::vector<double> buffer(frames * 2, 0.0);
    auto ctx = AudioContext { std::span(buffer.data(), buffer.size()), frames, sampleRate };
    dev.processAudio(ctx);

    const double f0 { 440.0 * std::pow(2.0, (60.0 - 69.0) / 12.0) };
    constexpr uint32_t window { 4096 };
    std::vector<double> levels;
    for (uint32_t start = frames / 4; start + window < frames; start += window) {
        levels.push_back(binMagnitude(buffer, start, window, f0, sampleRate));
    }
    QVERIFY(levels.size() > 8);
    ...
    const double fluctuation { std::sqrt(variance) / mean };
    QVERIFY2(fluctuation > 0.05, qPrintable(QString { "The strings held a flat level: %1" }.arg(fluctuation)));
}

void StringVoiceV3Test::test_strings_shouldShimmerRatherThanVibrate()
{
    // What the measured numbers alone did not catch. An earlier attempt gave the Strings the amount
    // of pitch movement the recordings show, but gave all of it to one oscillator, and a single
    // pitch swinging by that much is a vibrato -- which is not what the instrument does. On the
    // hardware the movement is spread across a cluster of components a few cents apart, so the
    // section shimmers while its centre of pitch stays put.
    //
    // So this asserts both halves: the fundamental is spread rather than a single line, and its
    // centre of mass barely moves. Either one alone would pass on something that sounds wrong.
    constexpr uint32_t sampleRate { 44100 };
    for (const uint8_t note : { uint8_t { 60 }, uint8_t { 84 } }) {
        StringVoiceV3Device dev { "Strings" };
        setUpStringsOnly(dev, sampleRate);
        dev.setEnsembleEnabled(false);
        dev.processMidiNoteOn(note, 100);
        ...
        // Spread: the copies sit StringsDetuneCents apart, so energy reaches well off the nominal
        // pitch. A single oscillator would leave the neighbouring bins far below the centre.
        constexpr uint32_t wide { 65536 };
        const double centre { binMagnitude(buffer, frames / 4, wide, f0, sampleRate) };
        const double offset { binMagnitude(buffer, frames / 4, wide, f0 * std::exp2(7.0 / 1200.0), sampleRate) };
        QVERIFY2(offset > centre * 0.25, qPrintable(QString { "Note %1 was a single line: %2 vs %3" }.arg(note).arg(offset).arg(centre)));

        // No vibrato: the centre of mass stays within a couple of cents across the whole render.
        constexpr uint32_t window { 8192 };
        std::vector<double> centroids;
        for (uint32_t start = frames / 4; start + window < frames; start += window) {
            centroids.push_back(fundamentalCentroidCents(buffer, start, window, f0, sampleRate));
        }
        QVERIFY(centroids.size() > 4);
        const auto [lo, hi] = std::minmax_element(centroids.begin(), centroids.end());
        const double swing { *hi - *lo };
        // The bound is well above the spread itself on purpose. Beating moves the centre of mass
        // around on its own, because which copy is loudest keeps changing, and that alone can walk
        // it across most of the detuning. What it cannot do is carry it beyond the copies: only
        // every component moving together does that, which is what a vibrato is. The attempt that
        // sounded wrong swung several times this far.
        QVERIFY2(swing < 30.0, qPrintable(QString { "Note %1 swung %2 cents, which is a vibrato" }.arg(note).arg(swing)));
    }
}

void StringVoiceV3Test::test_ensemble_shouldNotReachTheStrings()
{
    // The other half of the same fix: the Ensemble switch is wired to the Voice section, so throwing
    // it must leave a strings-only patch sounding exactly as it did.
    constexpr uint32_t sampleRate { 44100 };
    const auto renderStrings = [](bool ensembleEnabled) {
        StringVoiceV3Device dev { "Strings" };
        setUpStringsOnly(dev, sampleRate);
        dev.setEnsembleEnabled(ensembleEnabled);
        dev.processMidiNoteOn(60, 100);
        std::vector<double> buffer(8192 * 2, 0.0);
        auto ctx = AudioContext { std::span(buffer.data(), buffer.size()), 8192, sampleRate };
        dev.processAudio(ctx);
        return buffer;
    };

    const auto off = renderStrings(false);
    const auto on = renderStrings(true);
    QCOMPARE(off.size(), on.size());
    for (size_t i = 0; i < off.size(); i++) {
        QVERIFY2(std::abs(off[i] - on[i]) < 1.0e-12, qPrintable(QString { "Ensemble changed the strings at sample %1" }.arg(i)));
    }
}

void StringVoiceV3Test::test_strings_shouldSoundAtPitchWithALiftedFundamental()
{
...
void StringVoiceV3Test::test_stringsTone_shouldOpenAndCloseTheTopEnd()
{
...
void StringVoiceV3Test::test_balance_shouldScaleEachSectionIndependently()
{
...
void StringVoiceV3Test::test_formants_male_shouldPeakAtOhFrequencies()
{
...
void StringVoiceV3Test::test_formants_female_shouldNotNotchBetweenPeaks()
{
...
void StringVoiceV3Test::test_voiceStealing_shouldRemainStableWhenOversubscribed()
{
...
void StringVoiceV3Test::test_hpfAndLpf_shouldAttenuateSignal()
{
    auto renderRms = [](StringVoiceV3Device & dev) {
        std::vector<double> buffer(2048 * 2, 0.0);
        auto ctx = makeContext(buffer, 2048);
        dev.processAudio(ctx);
    ...
    StringVoiceV3Device openDev { "Open" };
    ...
    StringVoiceV3Device closedLpfDev { "ClosedLpf" };
    ...
    StringVoiceV3Device openHpfDev { "OpenHpf" };
...
void StringVoiceV3Test::test_polyphony_shouldNotRescaleHeldNotes()
{
    ...
    const auto render = [](std::initializer_list<uint8_t> notes) {
        StringVoiceV3Device dev { "Polyphony" };
        dev.setSampleRate(44100);
        dev.setStringsUpper(true);
        dev.setStringsLower(true);
        dev.setStringsAttack(0.0f);
...
void StringVoiceV3Test::test_velocitySensitivity_shouldScaleBothSections()
{
    ...
    const auto render = [](float sensitivity, uint8_t velocity, bool strings) {
        StringVoiceV3Device dev { "Velocity" };
        dev.setSampleRate(44100);
...
void StringVoiceV3Test::test_panSpread_shouldCreateStereoSeparation()
{
    StringVoiceV3Device dev { "Test StringVoice" };
    dev.setSampleRate(44100);
    dev.setStringsAttack(0.0f);
...
QTEST_GUILESS_MAIN(noteahead::StringVoiceV3Test)
