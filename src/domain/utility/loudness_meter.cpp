...
{
    return static_cast<float>(std::max(static_cast<double>(LoudnessMeter::MinimumLufs), powerToLufs(power)));
}

} // namespace

void LoudnessMeter::setActive(bool active)
{
...
bool LoudnessMeter::active() const
{
    return m_active.load();
}

void LoudnessMeter::setSampleRate(uint32_t sampleRate)
{
    m_sampleRate = static_cast<double>(sampleRate);
}

void LoudnessMeter::updateCoefficients()
{
...
double LoudnessMeter::applyKWeightL(double x)
{
...
double LoudnessMeter::applyKWeightR(double x)
{
...
void LoudnessMeter::advanceBlock(double meanPower)
{
...
size_t LoudnessMeter::histogramBin(double lufs)
{
...
void LoudnessMeter::accumulateGatingBlock(double meanPower)
{
...
void LoudnessMeter::applyPendingState()
{
...
void LoudnessMeter::accumulateSample(double left, double right)
{
...
void LoudnessMeter::processSample(double left, double right)
{
    if (m_sampleRate <= 0) {
        return;
    }
...
void LoudnessMeter::write(const double * interleavedStereo, uint32_t frameCount, uint32_t sampleRate)
{
    if (!m_active.load() || !frameCount || !sampleRate) {
        return;
    }

    // Once per buffer rather than per sample: the reset is an atomic read-modify-write, and a device
    // meter runs it over every frame of every device otherwise.
    setSampleRate(sampleRate);
    applyPendingState();

    for (uint32_t i = 0; i < frameCount; i++) {
        accumulateSample(interleavedStereo[i * 2], interleavedStereo[i * 2 + 1]);
    }
}
...
void LoudnessMeter::reset()
{
    ...
    m_absGatedCount = 0;
    m_momentaryLufs = MinimumLufs;
    m_shortTermLufs = MinimumLufs;
    m_integratedLufs = MinimumLufs;
}

void LoudnessMeter::requestReset()
{
