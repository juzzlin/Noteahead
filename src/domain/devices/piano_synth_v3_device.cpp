...
void PianoSynthV3Device::Voice::reset()
{
    string.reset();
    active = false;
  ...
  : m_name { std::move(name) }
{
    ...
    PianoSynthV3Device::syncParameters();
}

PianoSynthV3Device::~PianoSynthV3Device() = default;

std::string PianoSynthV3Device::name() const
{
    return m_name;
}

std::string PianoSynthV3Device::category() const
{
    return Constants::NahdXml::xmlValueSynths().toStdString();
}

std::string PianoSynthV3Device::typeName() const
{
    return Constants::pianoSynthV3DeviceName().toStdString();
}

std::string PianoSynthV3Device::typeIdString()
{
    return "291d51e6-a3a3-41be-b8d8-7035d7f114a8";
}

std::string PianoSynthV3Device::typeId() const
{
    return typeIdString();
}

std::vector<MidiCcController> PianoSynthV3Device::deviceMidiCcControllers() const
{
...
void PianoSynthV3Device::processMidiNoteOn(uint8_t note, uint8_t velocity)
{
    const std::lock_guard<std::recursive_mutex> lock { mutex() };
    handleNoteOn(note, velocity);
}

void PianoSynthV3Device::processMidiNoteOff(uint8_t note)
{
    const std::lock_guard<std::recursive_mutex> lock { mutex() };
    handleNoteOff(note);
}

void PianoSynthV3Device::processDeviceMidiCc(uint8_t controller, uint8_t value, uint8_t)
{
    using namespace MidiCcMapping;
...
void PianoSynthV3Device::processMidiAllNotesOff()
{
    const std::lock_guard<std::recursive_mutex> lock { mutex() };
    for (auto & v : m_voices) {
        if (v.active) {
...
void PianoSynthV3Device::processAudio(AudioContext & context)
{
...
bool PianoSynthV3Device::hasActiveAudio() const
{
    const std::lock_guard<std::recursive_mutex> lock { mutex() };
...
void PianoSynthV3Device::resetAudio()
{
    const std::lock_guard<std::recursive_mutex> lock { mutex() };
    for (auto & v : m_voices) {
        v.reset();
...
void PianoSynthV3Device::serializeToXml(ProjectWriter & writer) const
{
    const std::lock_guard<std::recursive_mutex> lock { mutex() };
...
void PianoSynthV3Device::deserializeFromXml(ProjectReader & reader)
{
    {
        const std::lock_guard<std::recursive_mutex> lock { mutex() };
...
ModalPianoStringV3::Settings PianoSynthV3Device::stringSettings() const
{
    return {
        m_brightness,
        m_decay,
...
void PianoSynthV3Device::releaseVoice(Voice & voice) const
{
...
int PianoSynthV3Device::allocateVoice()
{
    for (int i = 0; i < MaxVoices; i++) {
...
void PianoSynthV3Device::syncParameters()
{
...
float PianoSynthV3Device::brightness() const
{
    return m_brightness;
}

void PianoSynthV3Device::setBrightness(float brightness)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyBrightness().toStdString(), brightness);
}

float PianoSynthV3Device::decay() const
{
    return m_decay;
}

void PianoSynthV3Device::setDecay(float decay)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyDecay().toStdString(), decay);
}

float PianoSynthV3Device::inharmonicity() const
{
    return m_inharmonicity;
}

void PianoSynthV3Device::setInharmonicity(float inharmonicity)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyInharmonicity().toStdString(), inharmonicity);
}

float PianoSynthV3Device::hammerHardness() const
{
    return m_hammerHardness;
}

void PianoSynthV3Device::setHammerHardness(float hardness)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyHardness().toStdString(), hardness);
}

float PianoSynthV3Device::stringDetune() const
{
    return m_stringDetune;
}

void PianoSynthV3Device::setStringDetune(float detune)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyStringDetune().toStdString(), detune);
}

float PianoSynthV3Device::stretch() const
{
    return m_stretch;
}

void PianoSynthV3Device::setStretch(float stretch)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyStretch().toStdString(), stretch);
}

float PianoSynthV3Device::richness() const
{
    return m_richness;
}

void PianoSynthV3Device::setRichness(float richness)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyRichness().toStdString(), richness);
}

float PianoSynthV3Device::doubleDecay() const
{
    return m_doubleDecay;
}

void PianoSynthV3Device::setDoubleDecay(float doubleDecay)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyDoubleDecay().toStdString(), doubleDecay);
}

float PianoSynthV3Device::attack() const
{
    return m_attack;
}

void PianoSynthV3Device::setAttack(float attack)
{
...
void PianoSynthV3Device::setLpfCutoff(float cutoff)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyLpfCutoff().toStdString(), cutoff);
}

float PianoSynthV3Device::hpfCutoff() const
{
    return m_hpfCutoff;
}

void PianoSynthV3Device::setHpfCutoff(float cutoff)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyHpfCutoff().toStdString(), cutoff);
}

float PianoSynthV3Device::releaseTime() const
{
    return m_releaseTime;
}

void PianoSynthV3Device::setReleaseTime(float releaseTime)
{
    setContinuousParameterValue(Constants::NahdXml::xmlKeyReleaseTime().toStdString(), releaseTime);
}

float PianoSynthV3Device::stereoWidth() const
{
    return m_stereoWidth;
}

void PianoSynthV3Device::setStereoWidth(float stereoWidth)
{
