...
#ifndef STRING_VOICE_V3_DEVICE_HPP
#define STRING_VOICE_V3_DEVICE_HPP
...
class StringVoiceV3Device : public Device
{
public:
    explicit StringVoiceV3Device(std::string name);
    ~StringVoiceV3Device() override;
...
private:
    ...
    //! How many detuned copies the Strings section sounds each note as.
    //!
    //! This is what gives the section its shimmer, and it has to be more than one. A VC340's Strings
    //! move whether or not the Ensemble switch is engaged, but they do not move as one: what beats
    //! against what is a cluster of components a few cents apart, so no single pitch is heard to
    //! swing. Modulating one copy instead -- however faithfully it reproduces the measured amount of
    //! movement -- is a vibrato, which is not what the instrument does.
    static constexpr int StringUnisonCount { 3 };
    ...
    struct Voice
    {
        //! The Strings section sounds as several slightly detuned copies of itself rather than as one
        //! oscillator, which is where its shimmer comes from. See StringUnisonCount.
        std::array<PolyBlepOscillator, StringUnisonCount> stringOsc;
        //! Phase and rate of each copy's own slow tuning wander, one per copy so that no two of them
        //! drift together.
        std::array<double, StringUnisonCount> stringDriftPhase {};
        std::array<double, StringUnisonCount> stringDriftRateRatio {};
        //! Sine at the note's own frequency, lifting the fundamental above a saw's. See
        //! StringsFundamentalLift.
        std::array<PolyBlepOscillator, StringUnisonCount> stringFundamental;
    ...
    //! The Ensemble switch, which on this device reaches the Voice section only.
...
#endif // STRING_VOICE_V3_DEVICE_HPP
