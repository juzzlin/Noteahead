...
QtObject {
    ...
    signal copyAutomationDialogRequested(bool isPitchBend, int targetRow)
    ...
    signal songOverviewDialogRequested
    ...
    signal selectionPanInterpolationDialogRequested
    signal trackPanInterpolationDialogRequested
    ...
    function requestCopyEffectRackDialog(targetName: string): void {
        copyEffectRackDialogRequested(targetName);
    }
    ...
    function requestSongOverviewDialog(): void {
        songOverviewDialogRequested();
    }
    ...
    function requestSelectionPanInterpolationDialog(): void {
        if (!isPlaying()) {
            selectionPanInterpolationDialogRequested();
        }
    }
    function requestTrackPanInterpolationDialog(): void {
        if (!isPlaying()) {
            trackPanInterpolationDialogRequested();
        }
    }
    ...
    // No isPlaying() guard: this opens on top of an automation dialog that is already open, and
    // refusing it there would leave that dialog waiting for a picker that never comes.
    //
    // targetRow is the row of the edit dialog being filled, or -1 for the add form, which has no row
    // of its own until it is accepted.
    function requestCopyAutomationDialog(isPitchBend: bool, targetRow: int): void {
        copyAutomationDialogRequested(isPitchBend, targetRow);
    }
